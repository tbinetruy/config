...
(defun get-index-of-path (messages path)
  (let ((i 0)
        (result nil))
    (while (< i (length messages))
      (progn
        (if (string= path (alist-get 'path (nth i messages)))
           (setq result i)
         nil)
        (setq i (1+ i))))
    result))

(defun append-highlight-to-path (highlight-schema i loc)
  (let* ((current-schema (nth i highlight-schema))
         (current-highlights (append (car (alist-get 'highlights current-schema))
                                     loc))
         (current-path (alist-get 'path current-schema))
         (return-schema highlight-schema))
    (message "schema before: %s" current-schema)
    (message "current highlights: %s" (alist-get 'highlights current-schema))
    (setq current-schema `((path . ,current-path)
                           (highlights . (,current-highlights))))
    (message "schema after: %s" current-schema)
    (setf (nth i return-schema) current-schema)
    return-schema))

(defun get-highlight-schema (message extra-messages)
  (let ((messages (append `(,message) extra-messages))
        (highlight-schema nil)
        (i 0))
    (while (< i (length messages))
      (let* ((current-message (nth i messages))
             (current-path (alist-get 'path current-message))
             (index-of-path (get-index-of-path highlight-schema current-path))
             (current-highlights `(((line . ,(alist-get 'line current-message))
                                    (endline . ,(alist-get 'endline current-message))
                                    (start . ,(alist-get 'start current-message))
                                    (end . ,(alist-get 'end current-message))))))
        (if index-of-path
            (setq highligh-schema (append-highlight-to-path
                                   highlight-schema index-of-path
                                   current-highlights))
          (setq highlight-schema (append highlight-schema
                                         `(((path . ,current-path)
                                            (highlights . (,current-highlights)))))))
        (setq i (1+ i))))
    highlight-schema))
...
(defun show-code (err)
  (let* ((extra-messages (cdr (get-err-extras err)))
         ...
         (highlight-schema (get-highlight-schema message extra-messages)))
    (message "---> %s" highlight-schema)
    (mapcar (lambda (m)
